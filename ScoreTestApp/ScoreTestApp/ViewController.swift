...
class ViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate {
    ...
    @IBAction func buttonPressed(_ sender: Any) {
        ...
        scoreText.append(result[0])
        scoreText.append(result[1])
        
        print("text: ", scoreText)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let destinationVC = segue.destination as? PredictionVC else {return}
        destinationVC.str = scoreText
        //destinationVC.scoreLabel.text! = scoreText
    }
